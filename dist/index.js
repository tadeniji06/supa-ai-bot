...
bot.command('ratio', async (ctx) => {
    ...
    if (!Number.isFinite(value) || value < 0.1 || value > 5)
        return ctx.reply('❌ Ratio must be between 0.1 and 5. Example: <code>/ratio 1.0</code>', { parse_mode: 'HTML' });
...
bot.command('maxtrade', async (ctx) => {
    ...
    if (!Number.isFinite(value) || value <= 0)
        return ctx.reply('❌ Enter a positive USD amount. Example: <code>/maxtrade 100</code>', { parse_mode: 'HTML' });
...
bot.on('message:text', async (ctx, next) => {
    ...
    if (step === 'waiting_solana' || step === 'waiting_robinhood') {
        ...
        await User.findOneAndUpdate({ chatId: String(ctx.chat?.id) }, { chatId: String(ctx.chat?.id), username: ctx.from?.username, walletAddress: address, chain, lastSeenAt: new Date() }, { upsert: true, new: true });
        ctx.session.step = 'idle';
